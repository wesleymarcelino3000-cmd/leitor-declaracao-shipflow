...
function looksLikeDeclaration(text) {
  ...
  const hits = ['DECLARACAO', 'CONTEUDO', 'DESCRICAO', 'REMETENTE', 'DESTINATARIO', 'SHIPFLOW'].filter((word) => t.includes(word)).length;
...
function canonicalDescription(description) {
  const t = normalizeText(description)
    .replace(/[^A-Z0-9]+/g, ' ')
    .replace(/\b(UN|UND|UNIDADE|UNIDADES|PRODUTO|PRODUTOS)\b/g, '')
    .replace(/\s+/g, ' ')
    .trim();
  ...
  tableEl.innerHTML = rows.map((row) => `
    <div class="row">
      <div>
        ...
        <span>${row.count} etiqueta${row.count === 1 ? '' : 's'}</span>
      </div>
      <b>${row.count}</b>
    </div>
  `).join('');
